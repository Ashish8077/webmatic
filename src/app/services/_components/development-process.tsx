        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {data.steps?.map((step, index) => {
            ...
            return (
              <article
                key={step.key || index}
                className="group flex flex-col gap-5 rounded-2xl bg-white p-8 border border-slate-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 hover:border-orange-200 hover:shadow-orange-100/50"
              >
                {/* Icon */}
                <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-orange-50 text-orange-500 group-hover:bg-orange-500 group-hover:text-white transition-all duration-300 overflow-hidden">
